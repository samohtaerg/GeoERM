...
def main_execution_function(n_tasks_list=[20, 30, 40, 50, 60, 70, 80, 90, 100], n_samples=100, n_features=50, r=3, epsilon=0.1, h=0.5, n_iterations=1):
    # Define Models
    models = ['GeoERM', 'pERM', 'ERM', 'single_task_LR', 'pooled_LR', 'spectral', 'MoM', 'AdaptRep', 'GLasso']

    # Initialization
    total_results = {n_tasks: {"no_outliers": {model: [] for model in models},
    ...
    for iteration in range(n_iterations):
        ...
        # go through n_tasks value
        for n_tasks in n_tasks_list:
            print(f"Running for n_tasks = {n_tasks}...")

            # outlier and no outlier
            ...
            for model, mse_no_outlier, mse_with_outlier in zip(models, mse_results_no_outliers, mse_results_with_outliers):
                total_results[n_tasks]["no_outliers"][model].append(mse_no_outlier)
                total_results[n_tasks]["with_outliers"][model].append(mse_with_outlier)

    # Calculate average
    averaged_results = {n_tasks: {"no_outliers": {model: np.mean(total_results[n_tasks]["no_outliers"][model]) for model in models},
                                  "with_outliers": {model: np.mean(total_results[n_tasks]["with_outliers"][model]) for model in models}}
                        for n_tasks in n_tasks_list}
