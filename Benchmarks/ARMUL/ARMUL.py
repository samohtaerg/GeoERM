import numpy as np
from ARMUL.MTL import MTL, baselines, prediction, evaluation
from ARMUL.preprocessing import split_cv
...
def ARMUL_blackbox(data, r, eta = 0.02, L = 10, n_fold = 5, link = 'linear', seed = 0, c_max = 2):
    ...
    # set up parameters
    C_list = [(c_max * i / L) for i in range(1, L + 1)]
    lbd_list = [list(C * np.sqrt(p / n)) for C in C_list]

    test.cv(data, lbd_list, model = 'lowrank', K = r, n_fold = n_fold, seed = seed, eta_global = eta, eta_local = eta, eta_B = eta, eta_Z = eta, T_global = 500, intercept = False)
    beta_hat_ARMUL = test.models['lowrank'][:,:,0].T

    return(beta_hat_ARMUL)
