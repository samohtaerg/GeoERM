# -*- coding: utf-8 -*-
"""GeoERM HAR - LibMTL HPS + EW / PCGrad
...
for path in sys.path:
    # Fix 1: torchvision compatibility
    ...
    # Fix 2: Python 3.12 iterator .next() -> next()
...
class HAREncoder(nn.Module):
    def __init__(self, input_dim=561, hidden_dims=[256, 128, 64]):
        super().__init__()
        layers, d = [], input_dim
...
def run(seed=0, epochs=100, batch_size=64, lr=1e-3, weighting='EW', data_path=None):
    set_random_seed(seed)
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    print(f"Seed {seed} | Device: {device} | Weighting: {weighting}")

    with open(data_path, 'rb') as f:
        x, y, subject = pickle.load(f)
    ...
    train_loaders = {
        task_names[i]: DataLoader(
            HARTaskDataset(*train_sets[i]),
            batch_size=batch_size, shuffle=True, drop_last=True
        ) for i in range(T)
    }
    test_loaders = {
        task_names[i]: DataLoader(
            HARTaskDataset(*test_sets[i]),
            batch_size=batch_size, shuffle=False
        ) for i in range(T)
    }

    def encoder_class():
        return HAREncoder(input_dim=input_dim)

    decoders = nn.ModuleDict({
        t: HARDecoder(rep_dim=64, num_classes=2)
        for t in task_names
    })
...
if __name__ == '__main__':
    ...
    # ── USER CONFIGURATION ──────────────────────────────────────────────────
    DATA_PATH  = '/content/drive/MyDrive/RL-MTL-TL-main/datasets/2026/har_standardized.pkl'
    OUTPUT_DIR = '/content/drive/MyDrive/deepMTLoutput/libmtl_hps_ew'
    # ────────────────────────────────────────────────────────────────────────
