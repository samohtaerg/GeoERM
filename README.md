...
Implementation details for the experiments in the paper **"GeoERM: Geometry-Aware Multi-Task Representation Learning on Riemannian Manifolds"** by Aoran Chen and Yang Feng.
...
- **Penalized ERM (pERM)** (Tian et al., 2025): implemented using the public code from [https://github.com/ytstat/RL-MTL-TL](https://github.com/ytstat/RL-MTL-TL), included in `mtl_func_torch.py`.
- **Method-of-Moments (MoM)** (Tripuraneni et al., 2021): implemented in `mtl_func_torch.py` with default parameters.
- **ERM** (Du et al., 2020; Tripuraneni et al., 2021): implemented in `mtl_func_torch.py`.
- **Pooled Regression** (Ben-David et al., 2010; Crammer et al., 2008): implemented in `mtl_func_torch.py` using `sklearn.linear_model`.
- **Single-Task Regression**: implemented in `mtl_func_torch.py`. Estimates each task independently without sharing information across tasks. In the logistic setting, uses sklearn's default penalized logistic regression with within-task ridge-type shrinkage.
...
- **ARMUL** (Duan and Wang, 2023): implemented using the public code from [https://github.com/kw2934/ARMUL](https://github.com/kw2934/ARMUL), included in the `benchmarks/` folder, with default settings.
...
- Ensure `mtl_func_torch.py` and `funcs.py` are available in your working directory (both included in this repository).
- Download the UCI HAR Dataset from the [UCI Machine Learning Repository](https://archive.ics.uci.edu/dataset/240/human+activity+recognition+using+smartphones) and place it in the `real_data/HAR/UCI HAR Dataset/` folder.
- Data preprocessing: run `har_preprocessing.py` on your local machine or HPC, which produces `har_standardized.pkl`. Edit the `USER CONFIGURATION` paths at the top of the script before running.
- Experiment: run `geoerm_har.py --r <rank>` with random seed 0–99 (via `$SLURM_ARRAY_TASK_ID`) and rank values `r = 5, 10, 15`. Edit the `USER CONFIGURATION` paths at the top of the script before running.
...
- Data preprocessing: run one of the following scripts to generate the corresponding `.pkl` file. Edit the `USER CONFIGURATION` paths at the top of each script before running.
  - `mnist_preprocessing_45t.py` → `mnist_pairwise_50_45t.pkl` (45 tasks, identity only)
  - `mnist_preprocessing_90t.py` → `mnist_pairwise_50_90t.pkl` (90 tasks, identity + brightness)
  - `mnist_preprocessing_92t_adv.py` → `mnist_pairwise_50_92t_adv.pkl` (92 tasks, 90 + 2 adversarial)
- Experiment: run `geoerm_mnist_pairwise.py --dataset <dataset> --r <rank>` with random seed 0–99 (via `$SLURM_ARRAY_TASK_ID`), dataset values `45t, 90t, 92t_adv`, and rank values `r = 10, 15, 20`. Edit the `USER CONFIGURATION` paths at the top of the script before running.
